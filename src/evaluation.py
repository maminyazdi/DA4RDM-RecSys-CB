import pandas as pd
from sklearn.metrics import confusion_matrix, precision_recall_fscore_support, mean_squared_error, roc_curve, ndcg_score, classification_report ,auc
...
def evaluate(df):
    ...
    print('ground_truth_matrix.flatten() ', ground_truth_matrix.flatten())
    ...
    ground_truth_matrix_flat = ground_truth_matrix.flatten()
    similarity_matrix_flatten = similarity_matrix.flatten()
    # Compute ROC curve
    # fpr, tpr, _ = roc_curve(ground_truth_matrix.flatten(), similarity_matrix.flatten())
    ...
    roc_auc = dict()
    for i in range(5):
        fpr[i], tpr[i], _ = roc_curve(ground_truth_matrix_flat[:, i], similarity_matrix_flatten[:, i])
    ...
    # print("AUC score: {:.4f}".format(np.trapz(tpr, fpr)))
